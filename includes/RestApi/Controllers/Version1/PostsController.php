 ...
 * @version  2.6.0
 */
abstract class PostsController extends RestController {
	...
	public function get_item( $request ) {
		...
		if ( ! empty( $post->post_type ) && 'product_variation' === $post->post_type && 'product' === $this->post_type ) {
			return new \WP_Error( "masteriyo_rest_invalid_{$this->post_type}_id", __( 'To manipulate product variations you should use the /products/&lt;product_id&gt;/variations/&lt;id&gt; endpoint.', 'masteriyo' ), array( 'status' => 404 ) );
		} elseif ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
			return new \WP_Error( "masteriyo_rest_invalid_{$this->post_type}_id", __( 'Invalid ID.', 'masteriyo' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		 ...
		 */
		do_action( "masteriyo_rest_insert_{$this->post_type}", $post, $request, true );
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $post->post_type ) && 'product_variation' === $post->post_type && 'product' === $this->post_type ) {
			...
			return new \WP_Error( "masteriyo_rest_{$this->post_type}_invalid_id", __( 'ID is invalid.', 'masteriyo' ), array( 'status' => 400 ) );
		}
		 ...
		 * @param boolean         $creating  True when creating item, false when updating.
		 */
		do_action( "masteriyo_rest_insert_{$this->post_type}", $post, $request, false );
	...
	public function get_items( $request ) {
		 ...
		 */
		$args = apply_filters( "masteriyo_rest_{$this->post_type}_query", $args, $request );
	...
	public function delete_item( $request ) {
		...
		if ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
			return new \WP_Error( "masteriyo_rest_{$this->post_type}_invalid_id", __( 'ID is invalid.', 'masteriyo' ), array( 'status' => 404 ) );
		}
		 ...
		 */
		$supports_trash = apply_filters( "masteriyo_rest_{$this->post_type}_trashable", $supports_trash, $post );
		 ...
		 */
		do_action( "masteriyo_rest_delete_{$this->post_type}", $post, $response, $request );
	...
	public function get_collection_params() {
		...
		if ( 'wc/v1' === $this->namespace ) {
			$params['filter'] = array(
				'type'        => 'object',
				'description' => __( 'Use WP Query arguments to modify the response; private query vars require appropriate authorization.', 'masteriyo' ),
			);
		}
