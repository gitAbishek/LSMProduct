<?php
/**
 * The template for displaying user's courses.
 *
 * @version 0.1.0
 */

defined( 'ABSPATH' ) || exit;

?>

<div class="mycourses">
	<h2 class="mto-font-bold mto-text-2xl mto-text-textColor">My Courses</h2>
	<div class="mto-flex mto-flex-col md:mto-my-10 md:mto-flex-row mto-space-x-8">

		<div class="mto-flex-grow mto-border mto-rounded">
			<div class="mto-relative">
				<img class="mto-w-full" src="./img/mycourse1.jpg" alt="You are your only limit" />
			</div>

			<div class="course-detail mto-bg-white mto-p-6 mto-w-full">
				<div class="mto-mb-3">
					<span class="mto-inline-block">
						<svg class="mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M5.025 20.775A.998.998 0 006 22a1 1 0 00.555-.168L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082a1 1 0 00-.59-1.74l-5.701-.454-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.214 4.107-1.491 6.452zM12 5.429l2.042 4.521.588.047h.001l3.972.315-3.271 2.944-.001.002-.463.416.171.597v.003l1.253 4.385L12 15.798V5.429z"/>
						</svg>
					</span>

					<span class="mto-inline-block mto-bg-secondary mto-rounded-full mto-px-4 mto-py-1 mto-text-xs mto-uppercase mto-font-medium mto-text-white mto-ml-2">Book</span>
				</div>
				<h3 class="mto-font-bold mto-text-base mto-capitalize mb-2">Jango Courses</h3>
				<div class="mto-space-y-3 mto-my-6">
					<div class="mto-flex mto-justify-between">
						<div class="mto-inline-flex mto-justify-center mto-items-center">
							<svg class="mto-inline-block mto-fill-current mto-text-textColor mto-w-4 mto-mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
								<path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"/>
								<path d="M13 7h-2v6h6v-2h-4z"/>
							</svg>

							<time class="mto-inline-block mto-font-semibold mto-text-sm mto-text-textColor">10 hrs</time>
						</div>
						<div class="mto-inline-block mto-font-semibold mto-text-sm mto-text-textColor">
							15% Completed
						</div>
					</div>
					<div class="mto-my-4 mto-shadow mto-w-full mto-bg-primary-100 mto-rounded">
						<div class="mto-bg-primary mto-text-xs mto-leading-none mto-py-1 mto-text-center mto-text-white mto-rounded" style="width: 40%"></div>
					</div>
				</div>
				<div class="course-time-share mto-mt-4">
					<div class="mto-text-pColor mto-inline-block mto-text-sm">Started Jan 5, 2021</div>
					<div class="btn">
						<a href="#">Continue</a>
					</div>
				</div>
			</div>
		</div>

		<div class="mto-flex-grow mto-border mto-rounded">
			<div class="mto-relative">
				<img class="mto-w-full" src="./img/mycourse2.jpg" alt="You are your only limit" />
			</div>

			<div class="course-detail mto-bg-white mto-p-6 mto-w-full">
				<div class="mto-mb-3">
					<span class="mto-inline-block">
						<svg class="mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M5.025 20.775A.998.998 0 006 22a1 1 0 00.555-.168L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082a1 1 0 00-.59-1.74l-5.701-.454-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.214 4.107-1.491 6.452zM12 5.429l2.042 4.521.588.047h.001l3.972.315-3.271 2.944-.001.002-.463.416.171.597v.003l1.253 4.385L12 15.798V5.429z"/>
						</svg>
					</span>

					<span class="mto-inline-block mto-bg-secondary mto-rounded-full mto-px-4 mto-py-1 mto-text-xs mto-uppercase mto-font-medium mto-text-white mto-ml-2">Book</span>
				</div>
				<h3 class="mto-font-bold mto-text-base mto-capitalize mb-2">Jango Courses</h3>
				<div class="mto-space-y-3 mto-my-6">
					<div class="mto-flex mto-justify-between">
						<div class="mto-inline-flex mto-justify-center mto-items-center">
							<svg class="mto-inline-block mto-fill-current mto-text-textColor mto-w-4 mto-mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
								<path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"/>
								<path d="M13 7h-2v6h6v-2h-4z"/>
							</svg>

							<time class="mto-inline-block mto-font-semibold mto-text-sm mto-text-textColor">10 hrs</time>
						</div>
						<div class="mto-inline-block mto-font-semibold mto-text-sm mto-text-textColor">
							15% Completed
						</div>
					</div>
					<div class="mto-my-4 mto-shadow mto-w-full mto-bg-primary-100 mto-rounded">
						<div class="mto-bg-primary mto-text-xs mto-leading-none mto-py-1 mto-text-center mto-text-white mto-rounded" style="width: 40%"></div>
					</div>
				</div>
				<div class="course-time-share mto-mt-4">
					<div class="mto-text-pColor mto-inline-block mto-text-sm">Started Jan 5, 2021</div>
					<div class="btn">
						<a href="#" >Continue</a>
					</div>
				</div>
			</div>
		</div>

		<div class="mto-flex-grow mto-border mto-rounded">
			<div class="mto-relative">
				<img class="mto-w-full" src="./img/mycourse3.jpg" alt="You are your only limit" />
			</div>

			<div class="course-detail mto-bg-white mto-p-6 mto-w-full">
				<div class="mto-mb-3">
					<span class="mto-inline-block">
						<svg class="mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.947 9.179a1.001 1.001 0 00-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 001.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"/>
						</svg>
						<svg class=" mto-inline-block mto-fill-current mto-text-gray-800 mto-w-4 mto-h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M5.025 20.775A.998.998 0 006 22a1 1 0 00.555-.168L12 18.202l5.445 3.63a1.001 1.001 0 001.517-1.106l-1.829-6.4 4.536-4.082a1 1 0 00-.59-1.74l-5.701-.454-2.467-5.461a.998.998 0 00-1.822-.001L8.622 8.05l-5.701.453a1 1 0 00-.619 1.713l4.214 4.107-1.491 6.452zM12 5.429l2.042 4.521.588.047h.001l3.972.315-3.271 2.944-.001.002-.463.416.171.597v.003l1.253 4.385L12 15.798V5.429z"/>
						</svg>
					</span>

					<span class="mto-inline-block mto-bg-secondary mto-rounded-full mto-px-4 mto-py-1 mto-text-xs mto-uppercase mto-font-medium mto-text-white mto-ml-2">Book</span>
				</div>
				<h3 class="mto-font-bold mto-text-base mto-capitalize mb-2">Jango Courses</h3>
				<div class="mto-space-y-3 mto-my-6">
					<div class="mto-flex mto-justify-between">
						<div class="mto-inline-flex mto-justify-center mto-items-center">
							<svg class="mto-inline-block mto-fill-current mto-text-textColor mto-w-4 mto-mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
								<path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"/>
								<path d="M13 7h-2v6h6v-2h-4z"/>
							</svg>

							<time class="mto-inline-block mto-font-semibold mto-text-sm mto-text-textColor">10 hrs</time>
						</div>
						<div class="mto-inline-block mto-font-semibold mto-text-sm mto-text-textColor">
							15% Completed
						</div>
					</div>
					<div class="mto-my-4 mto-shadow mto-w-full mto-bg-primary-100 mto-rounded">
						<div class="mto-bg-primary mto-text-xs mto-leading-none mto-py-1 mto-text-center mto-text-white mto-rounded" style="width: 40%"></div>
					</div>
				</div>
				<div class="course-time-share mto-mt-4">
					<div class="mto-text-pColor mto-inline-block mto-text-sm">Started Jan 5, 2021</div>
					<div class="btn">
						<a href="#" >Continue</a>
					</div>
				</div>
			</div>
		</div>
	</div>
	<a class="btn mto-font-semibold mto-text-sm mto-text-pColor hover:mto-text-white mto-bg-white mto-border hover:mto-border-0" href="#">
		<span class="mto-inline-flex">Show All Courses</span>
		<svg class="mto-inline-flex mto-icon mto-fill-current mto--mt-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
			<path d="M10.707 17.707L16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"/>
		</svg>
	</a>

</div>

<div class="achivement-wrapper">
	<h2 class="mto-font-bold mto-text-2xl mto-text-textColor">My Achivements</h2>
	<div class="mto-notify-message">
		<span>You have no achievements yet. Enroll in course to get an achievements</span>
	</div>
</div>

<div class="study-certificates mto-space-y-8">
	<h2 class="mto-font-bold mto-text-2xl mto-text-textColor">My Certificates</h2>
	<div class="certificate-list">
		<ul class="mto-space-y-6">
			<li>
				<div class="mto-flex md:mto-flex-row mto-justify-between mto-items-center md:mto-space-x-4">
					<span class="mto-uppercase mto-font-bold mto-text-textColor mto-text-xs">Course</span>
					<span class="mto-uppercase mto-font-bold mto-text-textColor mto-text-xs">Certificates</span>
				</div>
			</li>
			<li>
				<div class="mto-flex md:mto-flex-row mto-justify-between mto-items-center md:mto-space-x-4">
					<div class="mto-inline-flex mto-items-center mto-space-x-4">
						<img class="mto-w-10" src="./img/certificate.jpg" alt="">
						<h3 class="mto-font-bold mto-text-textColor mto-text-base">Building a Better Software</h3>
					</div>
					<div>
						<a class="btn mto-uppercase" href="#">
							<span class="mto-inline-flex">Download</span>
						</a>
					</div>
				</div>
			</li>
			<li>
				<div class="mto-flex md:mto-flex-row mto-justify-between mto-items-center md:mto-space-x-4">
					<div class="mto-inline-flex mto-items-center mto-space-x-4">
						<img class="mto-w-10" src="./img/certificate.jpg" alt="">
						<h3 class="mto-font-bold mto-text-textColor mto-text-base">Building a Better Software</h3>
					</div>
					<div>
						<a class="btn mto-uppercase" href="#">
							<span class="mto-inline-flex">Download</span>
						</a>
					</div>
				</div>
			</li>
			<li>
				<div class="mto-flex md:mto-flex-row mto-justify-between mto-items-center md:mto-space-x-4">
					<div class="mto-inline-flex mto-items-center mto-space-x-4">
						<img class="mto-w-10" src="./img/certificate.jpg" alt="">
						<h3 class="mto-font-bold mto-text-textColor mto-text-base">Building a Better Software</h3>
					</div>
					<div>
						<a class="btn mto-uppercase" href="#">
							<span class="mto-inline-flex">Download</span>
						</a>
					</div>
				</div>
			</li>
		</ul>
	</div>
	<a class="btn mto-font-semibold mto-text-sm mto-text-pColor hover:mto-text-white mto-bg-white mto-border hover:mto-border-0" href="#">
		<span class="mto-inline-flex">Show All Certificates</span>
		<svg class="mto-inline-flex mto-icon mto-fill-current mto--mt-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
			<path d="M10.707 17.707L16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"/>
		</svg>
	</a>
</div>

<?php
