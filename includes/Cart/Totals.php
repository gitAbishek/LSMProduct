...
final class Totals {
	...
	protected function get_tax_class_costs() {
		...
		$costs                = array_fill_keys( $item_tax_classes + $fee_tax_classes, 0 );
		$costs['non-taxable'] = 0;
	...
	protected function get_fees_from_cart() {
		...
		foreach ( $this->cart->get_fees() as $fee_key => $fee_object ) {
			...
			if ( 0 > $fee->total ) {
				$max_discount = NumberUtil::round( $this->get_total( 'items_total', true ) + $fee_running_total ) * -1;
			...
			$fee->taxes     = apply_filters( 'masteriyo_cart_totals_get_fees_from_cart_taxes', $fee->taxes, $fee, $this );
	...
	protected function remove_item_base_taxes( $item ) {
		if ( $item->price_includes_tax && $item->taxable ) {
			if ( apply_filters( 'masteriyo_adjust_non_base_location_prices', true ) ) {
	...
	protected function get_item_tax_rates( $item ) {
		...
		return apply_filters( 'masteriyo_cart_totals_get_item_tax_rates', $item_tax_rates, $item, $this->cart );
	}
	 ...
	 * @return array
	 */
	protected function get_merged_taxes( $in_cents = false, $types = array( 'items', 'fees' ) ) {
		$items = array();
		$taxes = array();
	...
	*/
		...
		foreach ( $this->items as $item_key => $item ) {
			...
			if ( has_filter( 'masteriyo_get_discounted_price' ) ) {
				...
				$item->total = wc_add_number_precision(
					apply_filters( 'masteriyo_get_discounted_price', wc_remove_number_precision( $item->total ), $item->object, $this->cart )
				);
			}

			if ( $this->calculate_tax && $item->product->is_taxable() ) {
				$total_taxes     = apply_filters( 'masteriyo_calculate_item_totals_taxes', WC_Tax::calc_tax( $item->total, $item->tax_rates, $item->price_includes_tax ), $item, $this );
	...
	protected function calculate_totals() {
		$this->set_total( 'total', NumberUtil::round( $this->get_total( 'items_total', true ) + $this->get_total( 'fees_total', true ) + array_sum( $this->get_merged_taxes( true ) ), 0 ) );
