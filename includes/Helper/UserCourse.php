...
/**
 * Get an array containing list of enrolled courses and a list of all courses the user bought.
 *
 * @since 0.1.0
 *
 * @return array
 */
function masteriyo_get_enrolled_and_all_courses( $user_id ) {
	$query            = new UserCourseQuery(
		array(
			'user_id' => $user_id,
			'limit'   => -1,
	...
	foreach ( $user_courses as $user_course ) {
		...
		if ( $user_course->get_status() === 'enrolled' ) {
			$enrolled_courses[] = $course;
		}
		$all_courses[] = $course;
	}

	return compact( 'enrolled_courses', 'all_courses' );
}
